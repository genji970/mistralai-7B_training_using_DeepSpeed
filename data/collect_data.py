...
# 저장 경로 설정
save_dir = "C:/huggingface_dataset"
    ...
    for item in ds:
        try:
            ...
            for i, answer_text in enumerate(answer_list):
                ...
                # 주제 필터링
                if not should_sample_regex(context + " " + question, TOPIC_REGEX_PATTERNS):
                ...
                # 전처리
                output_filtered = extract_relevant_sentences(context, question, output_raw)
                output_cleaned = clean_git_answer(output_filtered)
                reward = compute_reward(context, question, output_cleaned)
